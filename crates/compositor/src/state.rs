...
pub struct AnvilState<BackendData: Backend + 'static> {
    ...
    /// Canvas viewport (pan + zoom). Single camera for now; per-output
    /// cameras come when multi-monitor canvas support lands.
    pub camera: crate::camera::Camera,
    /// True while MMB is held for canvas-pan. When set, motion events
    /// translate cursor delta into camera shift instead of forwarding to
    /// surfaces.
    pub canvas_panning: bool,
    /// Last seen cursor position **in output-local logical coords** during
    /// canvas-pan. Stable across map_output remaps (output size doesn't
    /// change, only its anchor in the Space) — using space-absolute coords
    /// here mixes reference frames and produces jitter.
    pub last_pan_local_pos: Option<Point<f64, Logical>>,
    /// Timestamp of the previous render frame, used to compute `dt` for
    /// `camera.step(dt)`. None on the very first frame — the step is
    /// skipped then so we don't try to interpolate against a zero-duration.
    pub last_animation_step: Option<smithay::utils::Time<smithay::utils::Monotonic>>,
    /// Per-window slide animations queued by `place_new_window`. Ticked
    /// from the udev render loop right after the camera step, on the same
    /// `dt`. See [`crate::window_animator`].
    pub window_animator: crate::window_animator::WindowAnimator,
    ...
    /// Previous keyboard-focus target stashed when the user triggers
    /// Super+Space to focus the tacet-launcher overlay. When Esc fires
    /// while chat is focused, we pop this back to the seat — gives the
    /// Spotlight/Raycast "summon, do thing, dismiss back to where I was"
    /// flow without forcing the user to alt-tab.
    pub focus_stack: Option<KeyboardFocusTarget>,
    /// Set inside the keyboard-input closure when Esc is intercepted
    /// while `focus_stack.is_some()`. We can't call `keyboard.set_focus`
    /// from inside `keyboard.input(...)` (keyboard is already borrowed),
    /// so we set this marker and the outer fn drains it after `input()`
    /// returns.
...
impl<BackendData: Backend> XdgDecorationHandler for AnvilState<BackendData> {
    // tacet runs frameless: we always declare Mode::ServerSide so
    // protocol-compliant clients (foot, alacritty, wezterm, Qt) skip
    // drawing their own headerbar, and our SSD path stays gated off
    // (see xdg.rs commit handler) so we draw no chrome either. The
    // window content fills the toplevel rect; drag is via Super+LMB.
    // Some toolkits (GTK, Electron) still draw headerbars regardless
    // of the protocol negotiation — that's a client choice the
    // compositor can't override.
...
impl<BackendData: Backend> AnvilState<BackendData> {
    /// Notify every visible surface that the effective HiDPI factor is
    /// `output_scale × camera.zoom`. Clients that implement
    /// `wp_fractional_scale_v1` (most Wayland-native apps) react by
    /// re-rendering at that resolution — buffers come back HiDPI, the
    /// compositor displays them 1:1 with our RescaleRenderElement zoom,
    /// and zoomed-in content stays crisp instead of upscaling-blurred.
    /// Called by input_handler::apply_camera_state on every camera change.
    pub fn broadcast_fractional_scale(&self) {
        // Use target_zoom (not current) so clients see a stable
        // preferred_scale for the duration of any in-progress animation
        // and only recompute their HiDPI buffer once at the start of
        // each user gesture instead of every frame. See Camera::target_zoom.
        let zoom = self.camera.target_zoom();
    ...
    fn new_fractional_scale(
        ...
        with_states(&surface, |states| {
            ...
            if let Some(output) = primary_scanout_output {
                with_fractional_scale(states, |fractional_scale| {
                    // Include camera.zoom so a new surface created while
                    // zoomed-in is told to render at the same HiDPI factor
                    // every other surface is using — otherwise it pops in
                    // blurry and only becomes crisp after the next zoom event.
                    fractional_scale.set_preferred_scale(
                        output.current_scale().fractional_scale() * self.camera.target_zoom(),
                    );
                });
            }
        });
...
impl<BackendData: Backend + 'static> AnvilState<BackendData> {
    pub fn init(
        ...
        seat.add_keyboard(
            XkbConfig {
                layout: "us,ru",
                options: Some("grp:alt_shift_toggle,grp:win_space_toggle".into()),
                ..XkbConfig::default()
            },
            200,
            25,
        ...
        AnvilState {
            ...
            last_pan_local_pos: None,
            last_animation_step: None,
            window_animator: crate::window_animator::WindowAnimator::default(),
            compositor_state,
            data_device_state,
            layer_shell_state,
...
impl<BackendData: Backend + 'static> AnvilState<BackendData> {
    ...
    pub fn post_repaint(
        ...
        // Capture camera zoom up-front: the per-frame fractional_scale
        // updates below run inside `self.space.elements().for_each(...)`
        // (borrows self.space) and inside surface-tree walks for layers /
        // cursor / dnd-icon. Touching self.camera from within those closures
        // would violate the borrow rules. We also need it here so the
        // preferred_scale we send each frame matches what
        // broadcast_fractional_scale sends on zoom events — otherwise the
        // anvil-derived per-frame code reverts our zoom-aware value to plain
        // output_scale, and the client receives oscillating preferred_scale
        // events that defeat HiDPI re-rendering on zoom-in.
        // Per-frame fractional_scale uses the camera's TARGET zoom (not
        // current) so smooth-zoom animations don't spam clients with
        // dozens of preferred_scale events that make foot/grid-clients
        // recompute their layout every frame and look like they're resizing.
        let camera_zoom = self.camera.target_zoom();
            ...
            window.with_surfaces(|surface, states| {
                ...
                if let Some(output) = primary_scanout_output.as_ref() {
                    with_fractional_scale(states, |fraction_scale| {
                        fraction_scale.set_preferred_scale(output.current_scale().fractional_scale() * camera_zoom);
                    });
                }
