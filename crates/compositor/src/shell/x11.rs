...
impl<BackendData: Backend> XwmHandler for AnvilState<BackendData> {
    ...
    fn map_window_request(&mut self, _xwm: XwmId, window: X11Surface) {
        ...
        // X11 clients require an exact bbox via `xsurface.configure(...)`
        // below, so we skip the slide animation here: animating would force
        // a reconfigure-per-frame and many X clients treat that as a real
        // resize request.
        place_new_window(&mut self.space, self.pointer.current_location(), &window, true, None);
        ...
        // Frameless: never enable smithay's default SSD headbar.
    ...
    fn unfullscreen_request(&mut self, _xwm: XwmId, window: X11Surface) {
            ...
            .find(|e| matches!(e.0.x11_surface(), Some(w) if w == &window))
        {
            ...
            // Frameless: keep SSD off on un-fullscreen too.
            elem.set_ssd(false);
