    ...
    fn ack_configure(&mut self, surface: WlSurface, configure: Configure) {
        if let Configure::Toplevel(configure) = configure {
            ...
            if let Some(window) = window {
                // tacet is frameless. We always declare ServerSide via
                // the xdg-decoration handler so clients skip CSD, but we
                // do not draw the smithay default headbar — leaving SSD
                // off here keeps the 32px header gone and disables its
                // pointer-target / drag-handle path in element.rs. Custom
                // chrome (if added later) goes in a separate render layer.
