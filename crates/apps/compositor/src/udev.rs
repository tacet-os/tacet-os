...
pub fn run_udev() {
    ...
    // Spawn session-level autostart processes (tacet-launcher, future
    // notification daemon, wallpaper, etc.) as compositor children so
    // they live + die with the session. Has to happen after the
    // Wayland socket is listening AND XWayland init, so spawned
    // processes find a valid WAYLAND_DISPLAY (and DISPLAY for X11
...
impl AnvilState<UdevData> {
    ...
    fn render(&mut self, node: DrmNode, crtc: Option<crtc::Handle>, frame_target: Time<Monotonic>) {
        ...
        // Advance the smooth pan/zoom interpolation toward its target
        // before this frame is rendered, so all the downstream consumers
        // (output remap, RescaleRenderElement, cursor scale, grid layout,
        // fractional_scale broadcast) see the in-progress state for THIS
        // frame. Once the camera reaches its target, is_animating() flips
        // false and we stop doing the per-frame apply_camera_state — the
        // compositor reverts to standard damage-driven repaint.
        //
        // Multi-output caveat: `render` is invoked once per (crtc, vblank)
        // event, so with N monitors the camera takes ~N steps per logical
        // frame. With exponential decay this just makes convergence ~N×
        // faster, not unstable. If it ever feels too snappy on multi-mon
        // we can guard step by "since last step ≥ refresh interval".
        if let Some(prev) = self.last_animation_step {
            let dt = Duration::from(frame_target).saturating_sub(Duration::from(prev));
            let dt_secs = dt.as_secs_f64();
            if self.camera.is_animating() {
                self.camera.step(dt_secs);
                // Camera position / zoom changed: re-map outputs to the new
                // anchor, push the new effective HiDPI scale to clients,
                // and force a full redraw so smithay's damage tracker
                // doesn't leave stale pixels from the previous frame.
                self.apply_camera_state();
            }
            // Slide newly-spawned windows from the pointer to the placement
            // grid slot picked by `place_new_window`. Same dt as the camera
            // so concurrent zoom+spawn motions stay in sync. The animator
            // overrides space.map_element each tick; it auto-clears once
            // every active slide has snapped to its target.
            if self.window_animator.is_animating() {
                self.window_animator.step(&mut self.space, dt_secs);
            }
        }
        self.last_animation_step = Some(frame_target);
...
fn render_surface<'a>(
    ...
    if output_geometry.to_f64().contains(pointer_location) {
        ...
        // pointer_location lives in canvas-coords (post-zoom transform).
        // The space's outputs are visually scaled by camera.zoom via
        // RescaleRenderElement, so the cursor's on-screen position must
        // be scaled the same way relative to the output framebuffer
        // origin — otherwise mouse and zoomed-window visuals drift apart
        // until they no longer overlap at zoom != 1.
        let cursor_canvas_rel = pointer_location - output_geometry.loc.to_f64();
        let cursor_pos = smithay::utils::Point::<f64, smithay::utils::Logical>::from((
            cursor_canvas_rel.x * camera.zoom,
            cursor_canvas_rel.y * camera.zoom,
        ));
